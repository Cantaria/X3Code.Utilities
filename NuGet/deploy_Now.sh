...
if test -f "$x3AzureUtils"; then
    dotnet nuget push $x3AzureUtils --source X3Get
else
...
if test -f "$x3Utils"; then
    dotnet nuget push $x3Utils --source X3Get
else
...
if test -f "$x3Infrastructure"; then
    dotnet nuget push $x3Infrastructure --source X3Get
else
...
if test -f "$x3Wasm"; then
    dotnet nuget push $x3Wasm --source X3Get
else
