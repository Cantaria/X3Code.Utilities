...
public static class DecimalExtension
{
    ...
    public static void ConfigureDecimalColum(this PropertyBuilder<decimal> property, int precision, int scale)
    {
        if (precision is <= 0 or > 38) throw new ArgumentOutOfRangeException(nameof(precision));
        if (scale < 0 || scale >= precision) throw new ArgumentOutOfRangeException(nameof(precision));
